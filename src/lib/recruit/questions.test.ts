...
describe('normalizeFollowUps', () => {
  it('老数据的纯字符串补成 purpose 为空', () => {
    expect(normalizeFollowUps(['当时 QPS 多少'])).toEqual([
      { purpose: '', question: '当时 QPS 多少' },
    ]);
  });

  it('新数据的对象原样保留', () => {
    expect(normalizeFollowUps([{ purpose: '要细节', question: '多少' }])).toEqual([
      { purpose: '要细节', question: '多少' },
    ]);
  });
...
describe('normalizeQuestions', () => {
  ...
  it('规整每一道题，其余字段原样带过', () => {
    ...
    expect(out![0].followUps).toEqual([{ purpose: '', question: '旧格式' }]);
