...
- **PostgreSQL Database** — One consistent database for local and production environments
...
See `.env.example` for all available options (Google OAuth, etc.).
...
| `pnpm db:generate` | Generate PostgreSQL Drizzle migrations |
| `pnpm db:generate:pg` | Generate PostgreSQL Drizzle migrations |
